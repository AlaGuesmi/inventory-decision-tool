...
<style>
    ...
    /* Metric cards — force bright white text */
    ...
    /* Divider */
    hr { border-color: #3a5a80 !important; margin: 28px 0 !important; }
...
def compute_policy(percentile, L, p, K, h_rate):
    ...
    return {'SS': round(SS, 0), 'ROP': round(ROP, 0),
            'EOQ': round(EOQ, 0), 'TC': round(TC, 2), 'CSL': percentile}
...
def make_plot(figsize=(12, 4)):
    """Dark background plot with clearly visible white axes and labels"""
    fig, ax = plt.subplots(figsize=figsize)
    fig.patch.set_facecolor('#1b2a4a')
    ax.set_facecolor('#243555')
    # White tick labels
    ax.tick_params(colors='#ffffff', labelsize=11, which='both')
    # White axis lines
    for spine in ax.spines.values():
        spine.set_edgecolor('#7ec8e3')
        spine.set_linewidth(1.5)
    # Subtle grid
    ax.grid(True, alpha=0.2, color='#7ec8e3', linestyle='--')
    return fig, ax
 
 
def section_header(title):
    st.markdown(f"""
    <div style='font-size:1.9rem; font-weight:800; color:#7ec8e3;
                border-bottom:2px solid #3a5a80; padding-bottom:10px;
...
# MANAGER INPUT
# ─────────────────────────────────────────
section_header("Manager Input")
section_desc("Enter the current inventory level and adjust cost parameters if needed. All inputs use research-validated defaults.")
 
col1, col2, col3, col4, col5 = st.columns(5)
with col1:
    current_inventory = st.number_input(
        "Current Inventory (units)", min_value=0, value=5000, step=100,
        help="Current number of units physically on hand")
with col2:
    lead_time = st.number_input(
        "Lead Time (days)", min_value=1, max_value=21, value=7,
        help="Days between placing and receiving an order")
with col3:
    unit_value = st.number_input(
        "Unit Value ($)", min_value=1, max_value=50, value=4, step=1,
        help="Average unit value of Personal Care products")
with col4:
    K = st.number_input(
        "Ordering Cost K ($)", min_value=10, max_value=200, value=50, step=5,
...
# ─────────────────────────────────────────
section_header("7-Day Demand Forecast")
...
ax1.fill_between(forecast_dates, lower_bound, upper_bound,
                 alpha=0.25, color='#5ba8f5', label='Uncertainty Band (5th–95th pct)')
ax1.set_xlabel('Date', color='#ffffff', fontsize=12, fontweight='bold')
ax1.set_ylabel('Forecasted Units Sold', color='#ffffff', fontsize=12, fontweight='bold')
ax1.set_title('7-Day Demand Forecast — Personal Care, Store 44',
              color='#ffffff', fontsize=14, fontweight='bold', pad=15)
ax1.legend(facecolor='#243555', edgecolor='#7ec8e3', labelcolor='#ffffff', fontsize=11)
...
section_desc("Three inventory policies derived simultaneously under different uncertainty assumptions. Select the scenario that best reflects your current operational context.")
 
col_opt, col_base, col_pes = st.columns(3)
with col_opt:
    st.markdown("""
    <div style='background-color:#1e3d2a; border:2px solid #2ecc71;
                border-radius:12px; padding:20px;'>
        <div style='color:#2ecc71; font-weight:800; font-size:1.2rem;
                    margin-bottom:8px; font-family:Open Sans,sans-serif;'>
            🟢 Optimistic — 90% CSL
        </div>
        <div style='color:#b2dfdb; font-size:1rem; line-height:1.6;
                    font-family:Open Sans,sans-serif;'>
            Stable conditions. No upcoming holidays, promotions, or payday events.
            Lean safety stock, lower holding cost.
        </div>
    </div>
    """, unsafe_allow_html=True)
with col_base:
...
with col_pes:
    st.markdown("""
    <div style='background-color:#3d1e1e; border:2px solid #e74c3c;
                border-radius:12px; padding:20px;'>
        <div style='color:#e74c3c; font-weight:800; font-size:1.2rem;
                    margin-bottom:8px; font-family:Open Sans,sans-serif;'>
            🔴 Pessimistic — 99% CSL
        </div>
        <div style='color:#f5b3b3; font-size:1rem; line-height:1.6;
                    font-family:Open Sans,sans-serif;'>
            Elevated demand expected. Upcoming holiday, payday, or promotion.
            Higher safety stock, maximum protection.
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def color_rows(row):
    ...
    c = row_bg[idx]
    return [f'background-color: {c}; color: #ffffff; font-size: 1.05rem; '
            f'text-align: center; padding: 14px; border: 1px solid #2e4a6a;'] * len(row)
 
styled = scenario_df.style.apply(color_rows, axis=1).set_table_styles([
    {'selector': 'th', 'props': [
...
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("""
<div style='font-size:1.3rem; font-weight:700; color:#b0d4e8;
            font-family:Open Sans,sans-serif; margin-bottom:12px;'>
    Projected Inventory Trajectory — Next 7 Days
</div>
""", unsafe_allow_html=True)
...
# ─────────────────────────────────────────
section_header("Replenishment Decision")
...
with col3:
    ...
    status = "above reorder point" if delta >= 0 else "below reorder point"
    st.markdown(f"""
    <div style='background-color:#0d1a2e; border:2px solid #7ec8e3; border-radius:14px; padding:20px;'>
        ...
        <div style='color:{delta_color}; font-size:1rem; font-weight:700; margin-top:6px; font-family:Open Sans,sans-serif;'>{arrow} {pct}% {status}</div>
    </div>""", unsafe_allow_html=True)
...
if current_inventory <= ROP:
    st.markdown(f"""
    <div style='background-color:#3d1e1e; border:2px solid #e74c3c;
                border-radius:14px; padding:28px; font-family:Open Sans,sans-serif;'>
        <div style='color:#e74c3c; font-weight:800; font-size:1.6rem; margin-bottom:14px;'>
            🔴 PLACE ORDER NOW
        </div>
        <p style='color:#f5c6c6; font-size:1.15rem; margin:10px 0;'>
            <b>Order Quantity:</b> {int(EOQ):,} units
        </p>
        <p style='color:#f5c6c6; font-size:1.15rem; margin:10px 0;'>
            <b>Expected Delivery:</b>
            {(LAST_DATE + timedelta(days=lead_time)).strftime('%Y-%m-%d')}
            ({lead_time} days)
        </p>
        <p style='color:#f5c6c6; font-size:1.15rem; margin:10px 0;'>
            <b>Projected inventory at delivery:</b>
            {max(0, int(current_inventory - sum(forecasts[:lead_time]))):,} units
        </p>
    </div>
    """, unsafe_allow_html=True)
else:
    ...
    if days_until_rop:
        st.markdown(f"""
        <div style='background-color:#3d3000; border:2px solid #f39c12;
                    border-radius:14px; padding:28px; font-family:Open Sans,sans-serif;'>
            <div style='color:#f39c12; font-weight:800; font-size:1.6rem; margin-bottom:14px;'>
                🟡 NO ORDER — MONITOR CLOSELY
            </div>
            <p style='color:#fdeaa7; font-size:1.15rem; margin:10px 0;'>
                Inventory above reorder point but may reach it within
                <b>{days_until_rop} days</b>.
            </p>
            <p style='color:#fdeaa7; font-size:1.15rem; margin:10px 0;'>
                Consider switching to the <b>Pessimistic</b> scenario and placing
                an order soon.
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style='background-color:#1e3d2a; border:2px solid #2ecc71;
                    border-radius:14px; padding:28px; font-family:Open Sans,sans-serif;'>
            <div style='color:#2ecc71; font-weight:800; font-size:1.6rem; margin-bottom:14px;'>
                🟢 NO ORDER NEEDED
            </div>
            <p style='color:#b2dfdb; font-size:1.15rem; margin:10px 0;'>
                Current inventory of <b>{current_inventory:,} units</b> is
                comfortably above the reorder point of <b>{int(ROP):,} units</b>.
            </p>
            <p style='color:#b2dfdb; font-size:1.15rem; margin:10px 0;'>
                Next review recommended in <b>{lead_time} days</b>.
            </p>
        </div>
        """, unsafe_allow_html=True)
...
# PARETO FRONTIER
# ─────────────────────────────────────────
section_header("Cost vs Service Level Trade-off")
section_desc("Each point on the curve represents a non-dominated inventory policy. Moving right increases service level protection but at a higher annual cost. The three scenario operating points are highlighted.")
...
ax3.plot(pareto_csl, pareto_cost, '-', color='#7ec8e3',
...
ax3.set_xlabel('Target Cycle Service Level (%)', color='#ffffff', fontsize=12, fontweight='bold')
ax3.set_ylabel('Expected Annual Inventory Cost ($)', color='#ffffff', fontsize=12, fontweight='bold')
ax3.set_title('Cost vs Service Level Trade-off — XGBoost Inventory Policy',
              color='#ffffff', fontsize=14, fontweight='bold', pad=15)
ax3.legend(facecolor='#243555', edgecolor='#7ec8e3', labelcolor='#ffffff', fontsize=11)
...
st.markdown("""
<div style='background-color:#243555; border:1px solid #3a5a80; border-radius:12px;
            padding:22px; margin-top:16px; font-family:Open Sans,sans-serif;'>
...
<div style='text-align:center; color:#6a8faf; font-size:0.95rem; padding:10px 0;
    ...
    Powered by XGBoost &nbsp;·&nbsp; Continuous Review (s,Q) Policy with EOQ
